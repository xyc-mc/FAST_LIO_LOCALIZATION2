...
void Preprocess::rs32_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
    // cout<<"RS32 handler"<<endl;
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    pcl::PointCloud<rs32_ros::Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    ...
    // cout << "points num before filtering: " << plsize << endl;
    ...
    // Since there is a lot of points with nan x,y,z values, above method might not be accurate. 
    // Since we are using the /lidar/point_cloud from rs lidar, we are having the timestamp.
    given_offset_time = true;
    ...
    if(feature_enabled)
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        // Filter out points with zero timestamps or NaN coordinates
        if (pl_orig.points[i].timestamp == 0.0 ||
            std::isnan(pl_orig.points[i].x) ||
        ...
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        float time = float(pl_orig.points[i].timestamp - pl_orig.points[0].timestamp);
        added_pt.curvature = time * time_unit_scale; // units: ms
    ...
    else
    {
      for (int i = 0; i < plsize; i++)
      {
        ...
        if (  pl_orig.points[i].timestamp == 0.0 ||
          ...
          std::isnan(pl_orig.points[i].z))
        {
          // std::cout << "Filtered point: "
          // << "timestamp=" << pl_orig.points[i].timestamp
          // << ", x=" << pl_orig.points[i].x
          // << ", y=" << pl_orig.points[i].y
          // << ", z=" << pl_orig.points[i].z
          // << std::endl;
            continue;
        }

        // std::cout << "RS32 Point: timestamp=" << pl_orig.points[i].timestamp
        //           << ", x=" << pl_orig.points[i].x
        //           << ", y=" << pl_orig.points[i].y
        //           << ", z=" << pl_orig.points[i].z << std::endl;
